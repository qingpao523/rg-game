...
### 代码审查修复（v1.2.0）：路径穿越 / 召唤物 NaN / 武器系统接线 / 配置单一事实源

按代码审查结论修复（用户明确排除鉴权与风控类问题），覆盖 P0-P3 全部功能性缺陷：

- **P0 路径穿越**：`serveGame` 先 decode 再 normalize 且只剥开头 `../`，导致 `js/..%2f..%2fserver%2fdata%2fplayers.json`、`..%2f.git%2fconfig` 可读任意仓库文件（实测 200）。新增 `server/safe-path.js`：resolve 后必须落在根目录内且禁止 `..`/点开头路径段；管理后台静态路径同样加固
- **P0 召唤物 NaN**：服务端 `skills.json` 的 `taoist_skull_enhance` 用 `atkMult/hpMult`，客户端 `enhanceMult()` 读 `.atk` → `1+undefined=NaN`，召唤物伤害/血量全 NaN 且会把敌人 HP 打成 NaN 永久杀不死。修复：`db.js` 默认技能改为直接 vm 执行 `js/config.js` 生成（单一事实源），`enhanceMult()` 兼容两种字段名且缺省按 0；线上 `skills.json` 已重新对齐并删除无客户端实现的 `thunder_storm`（行为无 handler，注入即崩溃）
- **P1 武器系统接线**：10 把武器此前只写 `Player._weaponEffect` 无任何消费点，合成形同虚设。已实现全部效果：桃木剑分裂、五雷令牌电弧区、无名刀剑气、村正妖刀击杀增伤、枯骨杖召唤上限、阿努比斯权杖双箭、冰晶法杖冻结、霜之哀伤领域半径、铁壁盾护盾、圣光战锤判定范围；无武器时正确清空避免残留
- **P1 加载器**：JS 单文件失败重试 2 次 + 10s 看门狗，永久失败跳过而非卡死在 0%
- **P1 存档迁移/转盘**：旧版 `talentPoints` 迁移条件基于默认值合并后的对象导致永不执行，改为基于原始存档判断；转盘"天赋点"奖励写入通用/专精双池；背包统计同步修正
- **P1 首局上报**：结算上报与存档同步共享登录 Promise，先登录再上报，新浏览器首局不再丢失
- **P2**：敌人管理接口新增字段白名单校验（`validateEnemy`）+ 客户端远程敌人数值兜底钳制；`GAME_VERSION` 由服务器按 js/ 内容指纹注入（不再手工 bump）；静态文件 ETag 改 stat 指纹（去掉整文件 MD5），删除死代码 `serveStaticCached`；`readBody` 超限返回 413；JSON 写入改临时文件+rename 原子写；run id 改 UUID 消除截断后撞 id
- **P3**：清理无消费点的 economy 冗余字段与 `unlockedWeapons`；新增 `tests/h5-contract.test.js` 契约测试（服务端默认技能 ≡ 客户端 config.js、safeJoin 防穿越、敌人校验）

**关键决策**: 服务端默认技能不再手工复制，而是以客户端 `js/config.js` 为唯一事实源动态生成——字段漂移这类"两处都要改"的 bug 从结构上消灭，并靠契约测试兜底。遗留：`taoist_raise_dead` 客户端数值（40%/6）与 `skill-balance-v1.md`（25%/4）存在历史分歧，本轮按客户端现行手感对齐，是否回归文档口径待产品确认。

### 管理平台对局记录清理与职业校验（v1.2.1）

- **对局记录全错根因**：`runs.json` / `leaderboards.json` / `players.json` 中混入了早期 AI 会话造的演示数据（`web_test123`、`34a4fd72`，stats 10 局但 runs 只有 2 条、天赋 id `g_dmg_1` 在当前天赋树不存在），管理平台展示的全是假记录
- **清理**：清空 runs/leaderboards，删除两个测试玩家，保留无数据的真实玩家 `0bfcb0b9`
- **职业白名单**：`/api/run/complete` 校验 `class_id` 必须为 5 个职业之一（taoist/samurai/pharaoh/ice_witch/crusader），wave/time/kills/evolutions 必须为非负有限数字，非法值直接 422——从根上杜绝职业写错
- **管理端时间显示**：runs/players 时间列由 UTC 字符串截取改为本地时区格式化（此前北京时间差 8 小时）；对局记录空列表显示"暂无对局记录"

### 修复对局上报链路：登录路径不一致导致所有真实对局丢失（v1.2.2）

- **根因**：客户端登录调用 `POST /api/auth/login`，服务端注册的是 `POST /api/auth` → 404；且服务端登录响应没有 `token` 字段，客户端 `_token` 永远为空 → `_reportToServer` / `syncToServer` 全部静默放弃。**不是"只能本地记录"**——是任何电脑（含本机）的真实对局都从未上报成功，管理平台此前只有手工造的演示数据
- **修复**：服务端抽离 `handleAuth` 同时注册 `/api/auth` 与旧路径别名 `/api/auth/login`，响应增加 `token`；客户端改调 `/api/auth`，并对旧服务端做 `local-` 前缀兜底（无 token 也能标记已登录）
- **验证**：端到端实测登录 → 上报对局 → 排行榜/管理平台可见 → 清理探针数据后恢复空表；新增契约测试防路径再次漂移

### 武器装备优先级与常驻提示（v1.2.3）

- **稀有优先**：同职业多把武器时按稀有度降序装备（普通 < 稀有 < 史诗），不再固定取数组第一把；已用真实 WEAPONS 数据逐职业验证
- **开局提示**：进入战斗 banner 副标题追加"装备 XX"（如"道士 · 敕雷执契者 · 装备 五雷令牌"）
- **HUD 常驻角标**：头像下方显示"⚔ 武器名"，战斗中随时可见

### 管理端内置小白说明（v1.2.4）

- 新增 `docs/admin-guide.md` 完整操作说明
- 管理端对应页面内置折叠式「📖 使用说明」：对局记录、技能管理、游戏配置、排行榜、怪物管理
- 技能编辑弹窗内嵌关键提示：「行为」不可乱改、等级数值表 JSON 规则与示例、留底提醒
- 管理端静态文件加 `no-cache`，保证界面更新后刷新即可看到，不被浏览器缓存卡住

### 管理端接入本地 Ollama AI 配置助手（v1.2.5）

- **浮窗**：管理端右下角 🤖 悬浮窗口，任意页面可开，支持模型切换、清空对话、流式打字机输出
- **知识库**：每次提问自动打包当前游戏配置 vN、全部技能定义、职业/主动技/进化、武器、怪物定义（服务端优先、缺失回退客户端内置），改完配置自动跟随
- **接口**：`GET /api/admin/chat/models` 列出本地 Ollama 可用模型 + 知识库元信息；`POST /api/admin/chat` 带 admin 鉴权，SSE 流式转发 Ollama 回复
- **默认模型**：`richardyoung/qwythos-9b-abliterated:IQ4_XS`（可用 `OLLAMA_MODEL` 环境变量覆盖；超时默认 5 分钟，`OLLAMA_TIMEOUT_MS` 可调）
- **实测**：问答「当前玩家初始血量是多少？想调低前期难度改哪几个配置？」正确引用 `player.hp=120`、波次/精英/哥布林当前值与修改建议（本地 9B 模型首答约 1-2 分钟，属正常）
- **提示词优化**：补充数值方向约束（如 baseInterval 越小怪越多），避免模型给反方向的修改建议

### 修复 AI 助手思考过程展示（v1.2.6）

- **根因**：qwythos-9b（qwen35 系）在 Ollama 原生接口的输出分两个字段——`message.thinking`（思考过程）与 `message.content`（最终回答）。此前只转发/展示 content，思考阶段前端一直停在「思考中…」且长时间看不到结果
- **修复**：服务端同时转发 thinking 与 content；前端思考过程实时显示为灰色斜体气泡（💭，限高 220px 可滚动），最终回答以独立气泡流式展示；模型只输出思考没有回答时给出明确提示；流中错误也会透传显示
- **实测**：同一模型提问「1+1 等于几」，16 秒返回：思考过程 474 字符 + 最终回答「2」，两者均正常展示

### AI 助手接入代码知识库（v1.2.7）

- **代码接入**：js/、server/、server/admin/、tools/、index.html 共 20 个代码文件建立索引；按问题自动分词检索（英文标识符 + 中文双字词），大文件只取命中行附近 ±8 行窗口，小文件整段注入，上限 22KB（`OLLAMA_CODE_KB_CHARS` 可调）
- **发现并修复上下文超限**：该模型在 Ollama 中默认上下文仅 8k，加入代码 KB 后请求被 400 拒绝（exceed_context_size_error）；已在请求中显式 `num_ctx: 64000`（模型原生支持 1M，`OLLAMA_NUM_CTX` 可调）
- **实测**：问「武器效果在哪个文件实现的？」正确回答 craft.js 定义 + skills.js/player.js 等实现位置，思考过程与回答均流式展示
- **提示词约束**：代码回答只引用检索片段中真实出现的文件/代码，不编造函数名与行号

### AI 助手上下文水位自动重置（v1.2.8）

- 新增 `server/chat-context.js`：按 2 字符/token 保守估算系统提示词 + 历史占用；达到上下文 70%（默认）时从最旧消息开始清理到 45% 安全水位，单条超长消息截断到 30% 以内
- 触发时向管理端推送提示「对话接近上下文上限，已自动清理较早内容；本次回答后开始新对话」，前端在本轮回答完成后自动清空历史
- 模型实际上下文按 Ollama /api/tags 查询并 clamp（默认 64k；`OLLAMA_NUM_CTX`、`OLLAMA_CTX_RESET_RATIO` 可调）
- 管理端知识库标签显示「上下文 64k（70% 自动重置）」；新增契约测试覆盖水位清理逻辑（5/5 通过）

### 修复管理端远端缓存错乱（v1.2.9）

- **症状**：https://rush.qingpao.fun/admin 上无机器人按钮或点击报 `AdminApp.toggleChat is not a function`（本地正常）
- **根因**：HTML 是 no-cache 每次拿新版，但 app.js/style.css 此前无缓存头，被浏览器/Cloudflare 边缘缓存了旧版 → 新 HTML + 旧 JS
- **修复**：管理端 HTML 输出时给 `/admin/app.js`、`/admin/style.css` 注入内容指纹查询串（`?v=md5`，内容变 URL 变），旧缓存 URL 天然失效；HTML 仍 no-cache + ETag
- **验证**：本地与远端 HTML 均引用 `app.js?v=93090dfa17`，远端版本化资源 200 且包含 `toggleChat`；契约测试 5/5

### 对局记录详情化（v1.3.1）

- **客户端上报扩展**：结算时除基础战绩外，新增上报——技能构筑（id/名称/等级/是否进化）、进场天赋（天赋点分配快照）、进场武器（本职业拥有列表 + 实际装备名与效果）、最终等级、最终生命/最大生命（`main.js` 报告构建 + `storage.js` 上报）
- **服务端存储**：`/api/run/complete` 接收并落盘新字段，旧记录无字段时管理端显示 "—" 兼容
- **管理端交互**：对局记录表格新增「详情」列，点击任意行展开/收起详情区，显示：
  - 基本信息：玩家 ID、职业、最终等级、最终生命、主导流派、对局时间
  - 技能构筑（每技能 Lv、进化标记）
  - 进场天赋（天赋 ID 映射中文名 + 等级，36 个天赋节点名称表）
  - 进场武器（拥有列表含稀有度）+ 实际装备与效果
- **验证**：模拟上报 ice_witch 9 波 321s 12 级，管理端 API 完整返回 level/skills/talents/weapons/hp；探针数据清理；远端版本化 app.js 已同步；契约测试 5/5

### 结算后玩家起名（v1.3.2）

- **游戏流程**：打完一局进入「处置报告」后，未设置名字的玩家弹出「给自己起个名字」输入框（确认 / 跳过，Enter 确认）；跳过则下一局结算继续提醒，名字最多 16 字
- **名字落地**：存入本地存档并随 `syncToServer` 同步到服务端玩家资料；排行榜立即用新名字显示——服务端在玩家改名后同步刷新该玩家在全部榜单中的显示名（新增 `LBDB.updateName`）
- **对局记录**：上报携带名字，run 记录存储 `player_name`；管理端对局记录列表与详情均展示玩家名（旧记录无名字显示 "—"）
- **修复**：`/api/player/sync` 中 `PlayerDB.update` 是原地 `Object.assign`，导致更新后旧名比较恒等、榜单改名不生效的 bug（先存旧名再比较）
- **验证**：改名链路 API/磁盘三者一致；真实战绩（web_ca 530s、web_2f 139s）与 5 条对局记录保留；契约测试 5/5
