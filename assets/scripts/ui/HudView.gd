...
func _process(_delta: float) -> void:
	if not is_instance_valid(GameSession):
		return
	hp_bar.value = GameSession.health / GameSession.max_health * 100.0 if GameSession.max_health > 0 else 0
	mana_bar.value = GameSession.mana / GameSession.max_mana * 100.0 if GameSession.max_mana > 0 else 0
	time_label.text = TimeService.get_elapsed_formatted()
	hp_bar.get_node("Label").text = "%d/%d" % [GameSession.health, GameSession.max_health]
	mana_bar.get_node("Label").text = "%d/%d" % [GameSession.mana, GameSession.max_mana]
