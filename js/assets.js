...
const Assets = {
  ...
  loadAll(onProgress, onDone) {
    ...
    const loadOne = (path, isEssential) => {
      ...
      // av=资源内容哈希：图片内容不变则 URL 不变走缓存，变了才重新下载（只下载更新的）
      img.src = 'assets/' + path + '?av=' + (window.ASSET_VER || window.GAME_VERSION || '1');
  ...
  _reload(path) {
    ...
    img.src = 'assets/' + path + '?av=' + (window.ASSET_VER || window.GAME_VERSION || '1') + '&r=' + (this._retryN = (this._retryN || 0) + 1);
    this.images[path] = img;
  },
